  ...
  # 4. Gráfico
 kernel1 <- ggplot(datos_cond_log, aes(x = log_cond)) +
    geom_histogram(
...
# =========================================================
# DENSIDAD KERNEL: MISMO h, DIFERENTES FUNCIONES KERNEL
# =========================================================

# Usamos el mismo ancho de banda óptimo (h_optimo) calculado antes,
# y lo mantenemos fijo mientras cambiamos la función kernel.

d_gauss <- density(x_log, bw = h_optimo, kernel = "gaussian")
d_epan  <- density(x_log, bw = h_optimo, kernel = "epanechnikov")
d_rect  <- density(x_log, bw = h_optimo, kernel = "rectangular")
d_tri   <- density(x_log, bw = h_optimo, kernel = "triangular")

df_kernels_log <- bind_rows(
  data.frame(x = d_gauss$x, y = d_gauss$y, Kernel = "Gaussiano"),
  data.frame(x = d_epan$x,  y = d_epan$y,  Kernel = "Epanechnikov"),
  data.frame(x = d_rect$x,  y = d_rect$y,  Kernel = "Rectangular"),
  data.frame(x = d_tri$x,   y = d_tri$y,   Kernel = "Triangular")
)

df_kernels_log$Kernel <- factor(df_kernels_log$Kernel, levels = unique(df_kernels_log$Kernel))

# Gráfico: histograma + las 4 densidades superpuestas
kernel2 <- ggplot(datos_cond_log, aes(x = log_cond)) +
  geom_histogram(
    aes(y = after_stat(density)),
    bins = 40,
    fill = "gray90",
  ...
  geom_line(
    data = df_kernels_log,
    aes(x = x, y = y, color = Kernel, linetype = Kernel),
    linewidth = 1.05
  ) +
  scale_color_manual(values = c("firebrick", "darkblue", "forestgreen", "darkorange")) +
  scale_linetype_manual(values = c("solid", "dashed", "dotdash", "dotted")) +
  labs(
    title = "Estimación Kernel de la Conductividad (Escala Log10)",
    subtitle = paste0("Comparación de funciones kernel con h fijo (h = ", round(h_optimo, 2), ")"),
    x = expression(log[10]*"(Conductividad en µS/cm)"),
    y = "Densidad",
    color = "Función Kernel:",
    linetype = "Función Kernel:"
  ) +
  theme_minimal(base_size = 12) +
  theme(legend.position = "top")
